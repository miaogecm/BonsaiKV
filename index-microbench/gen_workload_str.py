...
for line in f_load :
    ...
    if len(cols) > 0 and cols[0] == "INSERT":
        # f_load_out.write (cols[0] + " " + cols[2][4:] + ' ' + bytes(hash(cols[4][7:]) % ((sys.maxsize + 1) * 2)) + "\n")
        f_load_out.write (cols[0] + " " + cols[2][4:] + ' ' + cols[4][6:] + "\n")
f_load.close()
f_load_out.close()
...
for line in f_txn :
    ...
    if (cols[0] == 'SCAN') or (cols[0] == 'INSERT') or (cols[0] == 'READ') or (cols[0] == 'UPDATE'):
        ...
        if cols[0] == 'SCAN' :
            ...
            # f_txn_out.write (cols[0] + ' ' + startkey + ' ' + bytes(hash(cols[4][7:]) % ((sys.maxsize + 1) * 2)) + '\n')
            f_txn_out.write (cols[0] + ' ' + startkey + ' ' + cols[4][6:] + '\n')
        else :
...
if key_type == 'randint' :
    ...
    for line in f_load :
        ...
        f_load_out.write (cols[0] + ' ' + str(count) + ' ' + cols[2] + '\n')
        count += 1
    ...
    for line in f_txn :
        ...
        if cols[0] == 'SCAN' :
            ...
            keymap[int(cols[1])] = count
            f_txn_out.write (cols[0] + ' ' + str(count) + ' ' + cols[2] + '\n')
            count += 1
        elif cols[0] == 'UPDATE' :
            f_txn_out.write (cols[0] + ' ' + str(keymap[int(cols[1])]) + ' ' + cols[2] + '\n')
        else :
