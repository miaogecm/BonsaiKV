...
if [ "load" = "$1" ] ; then
  YCSB_COMMAND=-load
  YCSB_CLASS=site.benchmark_ycsb.Client
elif [ "run" = "$1" ] ; then
  YCSB_COMMAND=-t
  YCSB_CLASS=site.benchmark_ycsb.Client
elif [ "shell" = "$1" ] ; then
  YCSB_COMMAND=
  YCSB_CLASS=site.benchmark_ycsb.CommandLine
else
...
# Source checkout
  ...
  if ! ls "$YCSB_HOME"/core/target/*.jar 1> /dev/null 2>&1 || \
    ...
    if mvn -Psource-run -pl site.benchmark_ycsb:"$MVN_PROJECT" -am package -DskipTests; then
