...
#if 0
#define bp_print(fmt, args ...) fprintf(stdout, fmt, ##args);
#else
#define bp_print(fmt, args ...) do {} while(0);
#endif
...
{	
	...
	root->b_root = NULL;
	bp_print("b+tree of order %d\n", DEFAULT_TREE_ORDER);
	return root;
}

__attribute__((noline)) char *search_record(b_node *leaf, uint64_t target_key, bool flag) 
{
#ifndef B_SEARCH	
	int i;
	static int count;
...
char* bp_lookup(void *p_root, uint64_t key)
{	
	b_root_obj *_root;
	b_node *root, *leaf;
	char* val;
	...
	return val;
/*	if (!val) 
		bp_print("key %ld not found\n", key);
	else 
		bp_print("Record at %p -- key %ld, val %s\n", 
				val, key, val);*/
}
...
int bp_insert(void *p_root, uint64_t key, 
		char *val)
{	
	char *old_val;
	char *free_val;
